...
parser = argparse.ArgumentParser(description="Creates an rz build script")
...
parser.add_argument("--version", type=str, help="rz version to be displayed on the title screen")
...
if args.version:
    ...
    RZ_VERSION = f"{commit}-{date}"
    if tag_commit == commit and tag != "":
        RZ_VERSION = tag

RZ_BIN_ADDRESS = "0x80400060"
CFLAGS = f"-c -std=gnu11 -Wall -ffunction-sections -fdata-sections -O2 -fno-reorder-blocks -fdiagnostics-color -Isrc -Iinclude {' '.join([i for l in args.cflags for i in l])}"
CPPFLAGS = f"-DURL=github.com/Rainchus/mp1-RZ -DRZ_VERSION={RZ_VERSION} -DF3DEX_GBI_2 {' '.join([i for l in args.cppflags for i in l])}"
...
n.variable("rz_bin_addr", RZ_BIN_ADDRESS)
...
n.rule(
    "ld",
    command="$ld $ldflags -Wl,--defsym,start=$rz_bin_addr -Wl,-Map=$map $in -o $out $libs",
    description="LD $out",
)
...
n.rule("clean", command=f"rm -rf {BUILDDIR} rz-jp.z64 rz-us.z64 rz-US.wad rz-JP.wad romc")
...
class CFile:
    def __init__(self, ver: str, path: str):
        self.cppflags = CPPFLAGS + " " + f"-DMP1_VERSION={ver.upper()}"
        self.path = path
        self.o_path = path.replace(SRCDIR, f"{OBJDIR}/{ver.lower()}").replace(".c", ".o")

    def build(self) -> None:
        # Ensure the output directory exists
        os.makedirs(Path(self.o_path).parent, exist_ok=True)
...
for ver in VERSIONS:
    BIN = f"{BINDIR}/{ver}/mp1-rz.bin"
    ELF = f"{BINDIR}/{ver}/mp1-rz.elf"
    ...
    n.build(
        ...
        variables={"libs": f"-lmp1-{ver}", "map": f"{BUILDDIR}/rz-{ver}.map"},
    )
